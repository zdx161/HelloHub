...
static bool try_get_xbytes(const memory_t* memory, uintptr_t ptr, uint32_t* out_value, uint8_t bytes, uint32_t* cursor) {
    uint32_t data = 0;
    if (bytes > 4) {
        printf("can't read more than 4 bytes, trying to read %d\n", bytes);
        return false;
    }
...
static bool try_get_leb128(const memory_t* memory, uintptr_t ptr, uint32_t* out_value, uint32_t* cursor, bool sign_extend) {
    ...
    if (c * 7 > 32) {
       printf("%s: data exceeds expected 4 bytes maximum\n", __FUNCTION__);
       return false;
    }
    if (sign_extend) {
        if (buf & 0x40) {
...
static bool read_dwarf(const memory_t* memory, uintptr_t ptr, uint32_t* out_value, uint8_t encoding, uint32_t* cursor) {
    ...
    switch (encoding & 0xf) {
        ...
        case DW_EH_PE_sdata4:
            ...
            break;
        default:
            printf("unrecognized dwarf lower part encoding: 0x%x\n", encoding);
            return false;
    }
    ...
    switch (encoding & 0xf0) {
        ...
        case DW_EH_PE_datarel:
            ...
            break;
        default:
            printf("unrecognized dwarf higher part encoding: 0x%x\n", encoding);
            return false;
    }
    return true;
}
...
static uintptr_t find_fde(const memory_t* memory,
    ...
    if (!pc) {
        printf("find_fde: pc is zero, no eh_frame\n");
        return 0;
    }
    const map_info_t* mi = find_map_info(map_info_list, pc);
    if (!mi) {
        printf("find_fde: no map info for pc:0x%x\n", pc);
        return 0;
    }
    const map_info_data_t* midata = mi->data;
    if (!midata) {
        printf("find_fde: no eh_frame_hdr for map: start=0x%x, end=0x%x\n", mi->start, mi->end);
        return 0;
    }
    ...
    */
    if (eh_hdr_info.version != 1) {
        printf("find_fde: eh_frame_hdr version %d is not supported\n", eh_hdr_info.version);
        return 0;
    }
    /* Getting the data:
    ...
    */
    ...
    printf("find_fde: found %d FDEs\n", eh_hdr_info.fde_count);
    ...
    if (high < 0) {
        printf("find_fde: pc %x is out of FDE bounds: %x\n", pc, start);
        return 0;
    }
    c += high * 8;
    ...
    printf("pc 0x%x, ENTRY %d: start=0x%x, fde=0x%x\n", pc, high, start, fde);
    return fde;
}
...
static bool execute_dwarf(const memory_t* memory, uintptr_t ptr, cie_info_t* cie_info,
    ...
    switch ((dwarf_CFA)inst) {
        ...
        case DW_CFA_offset:
            ...
            printf("DW_CFA_offset: r%d = o(%d)\n", op, dstate->regs[op].value);
            break;
        case DW_CFA_restore:
            dstate->regs[op].rule = stack->regs[op].rule;
            dstate->regs[op].value = stack->regs[op].value;
            printf("DW_CFA_restore: r%d = %c(%d)\n", op, dstate->regs[op].rule, dstate->regs[op].value);
            break;
        case DW_CFA_nop:
            break;
        case DW_CFA_set_loc: // probably we don't have it on x86.
            ...
            if (offset < dstate->loc) {
                printf("DW_CFA_set_loc: attempt to move location backward\n");
                return false;
            }
            dstate->loc = offset * cie_info->code_align;
            printf("DW_CFA_set_loc: %d to 0x%x\n", offset * cie_info->code_align, dstate->loc);
            break;
        case DW_CFA_advance_loc1:
            if (!try_get_byte(memory, ptr, (uint8_t*)&offset, cursor)) return false;
            dstate->loc += (uint8_t)offset * cie_info->code_align;
            printf("DW_CFA_advance_loc1: %d to 0x%x\n", (uint8_t)offset * cie_info->code_align, dstate->loc);
            break;
        case DW_CFA_advance_loc2:
            if (!try_get_xbytes(memory, ptr, &offset, 2, cursor)) return false;
            dstate->loc += (uint16_t)offset * cie_info->code_align;
            printf("DW_CFA_advance_loc2: %d to 0x%x\n", (uint16_t)offset * cie_info->code_align, dstate->loc);
            break;
        case DW_CFA_advance_loc4:
            if (!try_get_xbytes(memory, ptr, &offset, 4, cursor)) return false;
            dstate->loc += offset * cie_info->code_align;
            printf("DW_CFA_advance_loc4: %d to 0x%x\n", offset * cie_info->code_align, dstate->loc);
            break;
        case DW_CFA_offset_extended: // probably we don't have it on x86.
            ...
            if (reg > DWARF_REGISTERS) {
                printf("DW_CFA_offset_extended: r%d exceeds supported number of registers (%d)\n", reg, DWARF_REGISTERS);
                return false;
            }
            dstate->regs[reg].rule = 'o';
            dstate->regs[reg].value = offset * cie_info->data_align;
            printf("DW_CFA_offset_extended: r%d = o(%d)\n", reg, dstate->regs[reg].value);
            break;
        case DW_CFA_restore_extended: // probably we don't have it on x86.
            ...
            if (reg > DWARF_REGISTERS) {
                printf("DW_CFA_restore_extended: r%d exceeds supported number of registers (%d)\n", reg, DWARF_REGISTERS);
                return false;
            }
            printf("DW_CFA_restore: r%d = %c(%d)\n", reg, dstate->regs[reg].rule, dstate->regs[reg].value);
            break;
        case DW_CFA_undefined: // probably we don't have it on x86.
            ...
            if (reg > DWARF_REGISTERS) {
                printf("DW_CFA_undefined: r%d exceeds supported number of registers (%d)\n", reg, DWARF_REGISTERS);
                return false;
            }
        ...
        case DW_CFA_register: // probably we don't have it on x86.
            ...
            if (reg > DWARF_REGISTERS || offset > DWARF_REGISTERS) {
                printf("DW_CFA_register: r%d or r%d exceeds supported number of registers (%d)\n", reg, offset, DWARF_REGISTERS);
                return false;
            }
            dstate->regs[reg].rule = 'r';
            dstate->regs[reg].value = offset;
            printf("DW_CFA_register: r%d = r(%d)\n", reg, dstate->regs[reg].value);
            break;
        case DW_CFA_remember_state:
            if (*stack_ptr == DWARF_STATES_STACK) {
                printf("DW_CFA_remember_state: states stack overflow %d\n", *stack_ptr);
                return false;
            }
            stack[(*stack_ptr)++] = *dstate;
            printf("DW_CFA_remember_state: stacktop moves to %d\n", *stack_ptr);
            break;
        case DW_CFA_restore_state:
            /* We have CIE state saved at 0 position. It's not supposed to be taken
               by DW_CFA_restore_state. */
            if (*stack_ptr == 1) {
                printf("DW_CFA_restore_state: states stack is empty\n");
                return false;
            }
            ...
            *dstate = stack[--*stack_ptr];
            dstate->loc = saveloc;
            printf("DW_CFA_restore_state: stacktop moves to %d\n", *stack_ptr);
            break;
        case DW_CFA_def_cfa:
            if (!try_get_uleb128(memory, ptr, &reg, cursor)) return false;
            if (!try_get_uleb128(memory, ptr, &offset, cursor)) return false;
        ...
        case DW_CFA_def_cfa_register:
            ...
            dstate->cfa_reg = reg;
            printf("DW_CFA_def_cfa_register: r%d\n", reg);
            break;
        case DW_CFA_def_cfa_offset:
            ...
            dstate->cfa_off = offset;
            printf("DW_CFA_def_cfa_offset: %x\n", offset);
            break;
        default:
            printf("unrecognized DW_CFA_* instruction: 0x%x\n", inst);
            return false;
    }
    return true;
}
...
static bool get_old_register_value(const memory_t* memory, uint32_t cfa,
    ...
    switch (dstate->regs[reg].rule) {
        case 0:
            ...
            printf("get_old_register_value: value of r%d is the same\n", reg);
            // for ESP if it's not updated by dwarf rule we assume it's equal to CFA
            if (reg == DWARF_ESP) {
                printf("get_old_register_value: adjusting esp to CFA: 0x%x\n", cfa);
        ...
        case 'o':
            ...
            if (!try_get_word(memory, addr, &newstate->reg[reg])) {
                printf("get_old_register_value: can't read from 0x%x\n", addr);
                return false;
            }
            printf("get_old_register_value: r%d at 0x%x is 0x%x\n", reg, addr, newstate->reg[reg]);
            break;
        case 'r':
            /* We don't have all registers in state so don't even try to look at 'r' */
            printf("get_old_register_value: register lookup not implemented yet\n");
            break;
        default:
            printf("get_old_register_value: unexpected rule:%c value:%d for register %d\n",
...
static bool update_state(const memory_t* memory, unwind_state_t* state,
    ...
    if (dstate->cfa_reg == DWARF_ESP) {
        ...
        printf("update_state: unexpected CFA register: %d\n", dstate->cfa_reg);
        return false;
    }
    printf("update_state: new CFA: 0x%x\n", cfa);
    /* Getting EIP. */
    ...
    printf("update_state: IP:  0x%x; restore IP:  0x%x\n", state->reg[DWARF_EIP], newstate.reg[DWARF_EIP]);
    printf("update_state: EBP: 0x%x; restore EBP: 0x%x\n", state->reg[DWARF_EBP], newstate.reg[DWARF_EBP]);
    printf("update_state: ESP: 0x%x; restore ESP: 0x%x\n", state->reg[DWARF_ESP], newstate.reg[DWARF_ESP]);
    *state = newstate;
    return true;
}
...
static bool execute_fde(const memory_t* memory,
    ...
    */
    ...
    if ((int32_t)fde_length == -1) {
        printf("execute_fde: 64-bit dwarf detected, not implemented yet\n");
        return false;
    }
    ...
    if (cie_offset == 0) {
        ...
        if (cie_offset != 0) {
           printf("execute_fde: can't find CIE\n");
           return false;
        }
    }
    printf("execute_fde: FDE length: %d\n", fde_length);
    printf("execute_fde: CIE pointer: %x\n", cie);
    printf("execute_fde: CIE length: %d\n", cie_length);
    ...
    */
    ...
    printf("execute_fde: CIE version: %d\n", cie_info->version);
    uint8_t ch;
    do {
        ...
        switch (ch) {
           ...
           default:
              printf("execute_fde: Unrecognized CIE augmentation char: '%c'\n", ch);
              return false;
              break;
        }
    } while (ch);
    ...
    printf("execute_fde: CIE code alignment factor: %d\n", cie_info->code_align);
    printf("execute_fde: CIE data alignment factor: %d\n", cie_info->data_align);
    if (cie_info->aug_z) {
    ...
    printf("execute_fde: FDE start: %x\n", dstate->loc);
    if (!read_dwarf(memory, fde, &fde_info->length, 0, &c)) {
        return false;
    }
    printf("execute_fde: FDE length: %x\n", fde_info->length);
    if (cie_info->aug_z) {
    ...
    while (c < fde_length + 4 && state->reg[DWARF_EIP] >= dstate->loc) {
        ...
        printf("IP: %x, LOC: %x\n", state->reg[DWARF_EIP], dstate->loc);
    }
...
static ssize_t unwind_backtrace_common(const memory_t* memory,
    ...
    for (size_t index = 0; returned_frames < max_depth; index++) {
        ...
        */
        if (!fde) {
            uint32_t ip;
            printf("trying to restore registers from stack\n");
            if (!try_get_word(memory, state->reg[DWARF_EBP] + 4, &ip) ||
                ip == state->reg[DWARF_EIP]) {
                printf("can't get IP from stack\n");
                break;
            }
            ...
            if (!try_get_word(memory, state->reg[DWARF_EBP], &state->reg[DWARF_EBP])) {
                printf("can't get EBP from stack\n");
                break;
            }
            printf("restore IP: %x\n", state->reg[DWARF_EIP]);
            printf("restore BP: %x\n", state->reg[DWARF_EBP]);
        ...
        printf("Stack: 0x%x ... 0x%x - %d bytes\n", frame->stack_top, state->reg[DWARF_ESP], frame->stack_size);
    }
