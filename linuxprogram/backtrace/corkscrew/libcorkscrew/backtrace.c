...
ssize_t unwind_backtrace(backtrace_frame_t* backtrace, size_t ignore_depth, size_t max_depth) {
    printf("Unwinding current thread %d.\n", gettid());
...
static void unwind_backtrace_thread_signal_handler(int n __attribute__((unused)), siginfo_t* siginfo, void* sigcontext) {
    if (!android_atomic_acquire_cas(gettid(), STATE_DUMPING, &g_unwind_signal_state.tid_state)) {
        ...
        printf("Received spurious SIGURG on thread %d that was intended for thread %d.\n",
...
ssize_t unwind_backtrace_thread(pid_t tid, backtrace_frame_t* backtrace,
    ...
    printf("Unwinding thread %d from thread %d.\n", tid, gettid());
    ...
    if (!sigaction(SIGURG, &act, &oact)) {
        ...
        if (tgkill(getpid(), tid, SIGURG)) {
            printf("Failed to send SIGURG to thread %d.\n", tid);
        } else {
            ...
            for (;;) {
                ...
                if (wait_millis--) {
                    printf("Waiting for thread %d to start dumping the stack...\n", tid);
                    usleep(1000);
                } else {
                    printf("Timed out waiting for thread %d to start dumping the stack.\n", tid);
                    break;
                }
            }
        }
        ...
        if (tid_state == tid) {
            if (!android_atomic_acquire_cas(tid, STATE_CANCEL, &g_unwind_signal_state.tid_state)) {
                printf("Canceled thread %d stack dump.\n", tid);
        ...
        while (tid_state == STATE_DUMPING) {
            printf("Waiting for thread %d to finish dumping the stack...\n", tid);
            usleep(1000);
            tid_state = android_atomic_acquire_load(&g_unwind_signal_state.tid_state);
        }
