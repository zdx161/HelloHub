...
static void dec_ref(map_info_t* milist, my_map_info_data_t* data) {
    if (!--data->refs) {
        printf("Freed my_map_info_list %p.\n", milist);
        free(data);
...
map_info_t* acquire_my_map_info_list() {
    ...
    if (g_my_map_info_list != NULL) {
        ...
        if (age >= MAX_CACHE_AGE) {
            printf("Invalidated my_map_info_list %p, age=%lld.\n", g_my_map_info_list, age);
            dec_ref(g_my_map_info_list, data);
            g_my_map_info_list = NULL;
        } else {
            printf("Reusing my_map_info_list %p, age=%lld.\n", g_my_map_info_list, age);
        }
    }
    ...
    if (g_my_map_info_list == NULL) {
        ...
        if (g_my_map_info_list != NULL) {
            printf("Loaded my_map_info_list %p.\n", g_my_map_info_list);
