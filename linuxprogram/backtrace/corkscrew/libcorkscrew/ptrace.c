...
bool try_get_word(const memory_t* memory, uintptr_t ptr, uint32_t* out_value) {
    printf("try_get_word: reading word at %p\n", (void*) ptr);
    if (ptr & 3) {
        printf("try_get_word: invalid pointer %p\n", (void*) ptr);
        *out_value = 0xffffffffL;
        return false;
    }
    if (memory->tid < 0) {
        if (!is_readable_map(memory->map_info_list, ptr)) {
            printf("try_get_word: pointer %p not in a readable map\n", (void*) ptr);
            *out_value = 0xffffffffL;
            return false;
        }
        ...
        *out_value = ptrace(PTRACE_PEEKTEXT, memory->tid, (void*)ptr, NULL);
        if (*out_value == 0xffffffffL && errno) {
            printf("try_get_word: invalid pointer 0x%08x reading from tid %d, "
                    "ptrace() errno=%d\n", ptr, memory->tid, errno);
            return false;
        }
        return true;
    }
