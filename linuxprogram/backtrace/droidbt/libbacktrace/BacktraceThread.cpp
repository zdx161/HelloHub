...
void ThreadEntry::Wait(int value) {
  ...
  while (wait_value_ != value) {
    ...
    if (ret != 0) {
      // mark for NE when use BACK_LOGW in signal hander
...
bool BacktraceThread::Unwind(size_t num_ignore_frames, ucontext_t* ucontext) {
   ...
   {  //discard when thread in D state----
    char filename_procstat[64];
    char tname[64];
    char threadState=0;
    sprintf(filename_procstat, "/proc/%d/stat", Tid());
    FILE *file;
    char buf[256], *open_paren, *close_paren;
    file = fopen(filename_procstat, "r");
    if (!file) 
    {
        BACK_LOGW("(!file) ");
        return false;
    ...
    {
       BACK_LOGW("!open_paren || !close_paren");
       return false;
    }
    *open_paren = *close_paren = '\0';
    strncpy(tname, open_paren + 1, 63);
    tname[64-1] = 0;
    ...
    sscanf(close_paren + 1, " %c %*d %*d %*d %*d %*d %*d %*d %*d %*d %*d "
          			...
          			&threadState);
    //BACK_LOGW("thread(name:%s) state: %c", tname,threadState);
    if( (threadState=='D') ||(threadState=='T') || (threadState=='t') )
    {
        BACK_LOGW("thread name:%s,PID:%d tid:%d state: %c", tname, Pid(),Tid(),threadState);
        BACK_LOGW("threadState=='D' or 'T' or 't' ");
        return false;
    }
  }	 //discard when thread in D state+++
  ...
  entry->Wait(1);
  if (errno == ETIMEDOUT) {
    BACK_LOGW("[HD]: entry->Wait(0) timeout %d:%d in %s", Pid(), Tid(), __func__);
  }
